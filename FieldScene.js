...
var FieldStrategyDropDelete = function(parent, fallNewDrop, recordPlay){
  ...
  this.update = function(){
    ...
    // case Mode.BOMB_CHECK:self.updateBombCheck();break;
    ...
    try{
      ...
      if (deleteLists.length != 0){
        for(var i = 0 ; i < deleteLists.length ; ++ i){
          if (checkBalls[deleteLists[i][0].x + deleteLists[i][0].y * parent.hNum].color == 9){
            for(var j = 0 ; j < deleteLists[i].length ; ++ j){
              bombDeleteLists.push(deleteLists[i][j].x + deleteLists[i][j].y * parent.hNum);
            }
          }
        }
      }

      // deal with bomb first
      var bombList = [];
      for(var x = 0 ; x < parent.hNum ; ++ x){
        for(var y = 0; y < parent.vNum / 2 ; ++ y){
          ...
          if (parent.balls[n].color == 9){

            for(var xx = 0 ; xx < parent.hNum ; ++ xx){
              bombList.push([xx, y]);
            }
            for(var yy = 0 ; yy < parent.vNum / 2 ; ++ yy){
              bombList.push([x, yy]);
            }
          }
        }
